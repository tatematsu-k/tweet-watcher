...
def test_get_available_credential():
    with patch('boto3.resource') as mock_resource:
        ...
        mock_table.scan.return_value = {'Items': [{'bearer_token': 'a', 'latelimit_reset_time': None}]}
        ...
        mock_table.scan.return_value = {'Items': [{'bearer_token': 'b', 'latelimit_reset_time': past}]}
        ...
        mock_table.scan.return_value = {'Items': [{'bearer_token': 'c', 'latelimit_reset_time': future}]}
        assert repo.get_available_credential() is None
        # latelimit_reset_timeが不正な値
        mock_table.scan.return_value = {'Items': [{'bearer_token': 'd', 'latelimit_reset_time': 'invalid'}]}
        assert repo.get_available_credential() is None